...
Persistent intake commits processed-message results, audit events, and case state atomically. Commercial booking, quote, and payment-request records join the same transaction. SMS, CRM webhook, and follow-up delivery happen after that transaction as best-effort effects: they are not physically exactly-once and can be lost or duplicated around a process failure. A future transactional outbox plus provider idempotency keys is required for durable external delivery.
