...
Детерминированный AI-движок **замкнутого цикла продаж**: автоматизация
бизнес-процесса от входящего лида до сделки, **полная замена человеку, который
сидит и обрабатывает лиды**. Отработка возражений, подстройка под клиента,
техники продаж и анализ текущего диалога — детерминированные правила
(`sales_playbook` / `decision_router` / `qualification_service`). AI не выбирает
ход, цену и эскалацию: он только переформулирует уже одобренное внутри сценария
(«Business DNA»). Self-serve SaaS, Starter $199/мес, Pro $499/мес, 7 дней триала.

**Flywheel продаёт сам себя тем же циклом.** Входящий запрос на демо или триал —
обычный лид продукта, и он должен дойти до записи. Лиды и клиенты находятся
использованием самого продукта, не отдельным ручным сейлзом.

### УТП — и две частые подмены

**Ядро: продукт ведёт лида по всему циклу от обращения до сделки, на любом бизнесе,
без индивидуальной настройки.** Не CRM, не intake, не «снимаем контакты и передаём человеку».
