...
Детерминированный движок автоматизации **процесса продажи** — той работы,
которую обычно делает менеджер. Лид проходит этапы цикла **без участия человека
на каждом шаге**. AI не принимает решений: `decision_router` / `qualification_service`
решают, что сказать, AI только переформулирует уже одобренное («Business DNA»).
Один и тот же цикл для любого бизнеса: меняется каталог и формулировки, не стадии.
Self-serve SaaS, Starter $199/мес, Pro $499/мес, 7 дней триала. Сам Flywheel
должен продаваться тем же замкнутым циклом (виджет на сайте Flywheel).
...
- **Платёж на границе цикла**: клиент подтверждает оплату в диалоге («I paid»), sweep закрывает визит и шлёт отзыв. Кнопки на дашборде — только если шаг случился вне чата. Списания картой нет. Сам Flywheel ещё не продаётся своим виджетом (нет tenant DNA на лендинге).
