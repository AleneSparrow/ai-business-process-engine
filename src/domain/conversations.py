...
class Conversation:
    ...
    def set_status(self, status: ConversationStatus, occurred_at: datetime) -> None:
        allowed = {
            ...
            # method's own conversation.set_status(AI_ACTIVE, ...) call was
            # deterministically raising ValueError here on every single
            # reactivation attempt, in production, because CLOSED was
            # previously fully terminal at this layer -- the LOST-dead-end
            # fix's hand-trace checked state_machine.py's ProcessState
            # transitions (LOST -> REACTIVATION -> CONTACTED) but missed that
            # ConversationStatus has this separate transition table. The
