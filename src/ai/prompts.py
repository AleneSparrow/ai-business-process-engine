...
def intent_prompt(*, context: Mapping[str, Any], customer_message: str) -> Prompt:
    return Prompt(
        ...
        "urgency calibration: normal is the default for an ordinary service problem, even when the problem is "
        "inconvenient, expensive, concerns a child, follows a storm, or deserves a prompt business response. "
        "Use high only when the CUSTOMER explicitly communicates time pressure with words such as urgent, ASAP, "
        "right away, immediately, today, tonight, or cannot wait. Use emergency only for an explicit threat to "
        "health or safety. Do not infer high urgency merely from a repair symptom, a medical topic, a legal topic, "
        "or another serious-sounding industry noun.\n"
        ...
        "A clear request involving a child or family member is not by itself ambiguous, dangerous, or an advice "
        "request. A clear request describing property damage after weather is not by itself an emergency. In both "
        "cases, keep requires_human=false unless the customer's actual words independently meet one of the four "
        "conditions above.\n"
        ...
        "unintelligible: true ONLY when the current customer message carries no interpretable request or "
        "answer at all -- random characters, a keyboard-mashed string, gibberish, a meaningless fragment. "
        "False for ANY message that expresses a clear request or answer, even a short or incomplete one -- "
        "a bare service name, 'yes', a phone number, a name, or a message that is simply missing details "
        "still means unintelligible=false; missing details are handled by qualification questions, not by "
        "this field. Also false whenever the message is an emergency, hostile, or an explicit advice "
        "request that sets requires_human=true above -- those are comprehensible messages that need a "
        "human, never unintelligible ones. requires_human and unintelligible are mutually exclusive: never "
        "set both true for the same message. Independent of confidence -- do not lower confidence merely "
        "because unintelligible is true, and do not set unintelligible merely because confidence is low.\n"
