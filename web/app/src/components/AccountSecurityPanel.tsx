...
export function AccountSecurityPanel({ token }: { token: string }) {
  ...
  return <section className="max-w-2xl space-y-7">
    <div><h2 className="text-lg font-semibold">Security</h2><p className="text-sm text-[#6B6459] mt-1">Control access to your Evorove account. Secrets and recovery codes are never shown again after you leave this page.</p></div>
    ...
    <div className="border border-[#E7E5DE] rounded-xl p-5"><h3 className="font-semibold">Two-factor authentication</h3><p className="text-sm text-[#6B6459] mt-1 mb-4">{status?.two_factor_enabled ? `Enabled · ${status.recovery_codes_remaining} recovery codes remaining` : "Not enabled"}</p>{!status?.two_factor_enabled && !setup && <div className="grid gap-3"><Field label="Current password"><input type="password" required className={inputCls} value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} /></Field><button onClick={startSetup} className="justify-self-start px-4 py-2 rounded-lg text-sm text-white" style={{ backgroundColor: "#151515" }}>Set up authenticator app</button></div>}{setup && <form onSubmit={confirmSetup} className="mt-4"><p className="text-sm mb-2">Add this one-time key in Google Authenticator, 1Password, Authy, or another TOTP app:</p><code className="block break-all text-xs bg-[#F7F1E4] p-3 rounded">{setup.secret}</code><p className="text-xs text-[#6B6459] mt-2">Or use this provisioning URI in a compatible app: {setup.provisioning_uri}</p><Field label="Six-digit code"><input required className={inputCls} value={code} onChange={(e) => setCode(e.target.value)} /></Field><button className="px-4 py-2 rounded-lg text-sm text-white" style={{ backgroundColor: "#151515" }}>Confirm two-factor authentication</button></form>}{status?.two_factor_enabled && <div className="mt-4 grid gap-3"><Field label="Current password"><input type="password" className={inputCls} value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} /></Field><Field label="Authenticator or recovery code"><input className={inputCls} value={code} onChange={(e) => setCode(e.target.value)} /></Field><div className="flex gap-2"><button onClick={rotateCodes} className="px-3 py-2 rounded-lg text-sm border border-[#D8D4CB]">Generate new recovery codes</button><button onClick={disable} className="px-3 py-2 rounded-lg text-sm text-[#B4483A] border border-[#E7C6C1]">Disable 2FA</button></div></div>}</div>
