...
Milestone 6 adds persisted anonymous website conversations and the first customer-facing widget. Qualification, service-area enforcement, state transitions, booking/payment permissions, and escalation remain deterministic. Staff authentication/UI, third-party integrations, payment processing, and non-web outbound delivery are not included yet.
...
Milestone 6 adds these anonymous public routes:
...
The continuation token is 256 bits of URL-safe randomness. Only its SHA-256 hash is stored. Lookup always includes `business_id`; tokens expire after `PUBLIC_CONVERSATION_TOKEN_TTL_HOURS` and support server-side revocation. Public responses expose only the token, public conversation/case state, handoff flag, and rendered message history—never lead/case database IDs, Business DNA, audit events, prompts, provider metadata, or internal notes.
...
The AI receives at most eight recent redacted customer/assistant messages, selected structured facts, unresolved field/question identifiers, current case state, and the existing minimal Business DNA subset. Phone and email values are redacted from prior-message context. Strong existing service, location, time, identity, and qualification-answer facts are preserved; contradictory follow-up facts request human review instead of silently overwriting them. Once a case is `NEEDS_HUMAN`, `QUALIFIED`, `LOST`, `CANCELLED`, or `COMPLETED`, the conversation records later messages but does not restart autonomous qualification.
...
Every tenant-owned repository lookup requires `business_id`, and composite foreign keys prevent cases, conversations, messages, and events from crossing tenant ownership. Website intake runs inside one Unit of Work: the conversation message, database message claim, lead/case writes, audit events, state transition, assistant response, question tracking, and stored result commit or roll back together.
...
Add authentication and authorization around private tenant/staff routes, replace the process-local public rate limiter for multi-worker deployments, then introduce an action-executor boundary with an outbox. Outbound messaging and other real integrations remain later milestones.
